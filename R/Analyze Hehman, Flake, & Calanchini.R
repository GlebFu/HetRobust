...
HFC2 <- read_csv("data/HFC2.csv")

HFC_fit <- lm(DisproportionateBlackDeath ~ BlackWeaponAssoc + 
                WhiteImplicit + WhiteExplicit + 
                BlackImplicit + BlackExplicit + 
                BlackMedianIncome + WhiteMedianIncome + 
                PcentHSdegreeBlack + PcentHSdegreeWhite + 
                PcentBAdegreeBlack + PcentBAdegreeWhite + 
                Segregation + 
                VioCrime + Unemployment + 
                PopulationDensity + TotalLethalForceRate, data = HFC2)
