...
One well-known approach to inference in this setting is based on heteroskedasticity-consistent covariance matrix estimators (HCCMEs), which yield asymptotically consistent estimates of the sampling variance of OLS coefficient estimates under quite general conditions. 
...
The HCCMEs studied in this paper are a special case of the general class of cluster-robust covariance matrix estimators (CRCMEs), also known as sandwich estimators or linearization estimators, which are commonly used in regression analysis of multi-stage survey data \citep{skinner1989domain}, econometric panel data models \citep{Arellano1987computing, white1984asymptotic}, and generalized estimating equations for longitudinal data \citep{Liang1986longitudinal}. 
...
\citet{Bell2002bias} developed a similar test (also based on a Satterthwaite approximation)
...
In the present context, one candidate working model is to assume that the errors are homoskedastic, so that $\bs\Sigma = \sigma^2 \bm{I}$.
Under homoskedasticity, the degrees of freedom reduce to 
\begin{equation}
\label{eq:nu_hom}
...
\citet{Kauermann2001note} proposed two confidence intervals for $\bm{c}'\bs{\hat\beta}$ based on an Edgeworth approximation to the distribution of $T^{HC}$. 
...
Kauermann and Carroll focus on the HC2 variance estimator and calculate its degrees of freedom based on the working assumption that the errors are actually homoskedastic, as in Equation (\ref{eq:nu_hom_HC2}). An alternative would be to use the empirical degrees of freedom estimate, $\nu_E$, from Equation(\ref{eq:nu_empirical}). 

\citet{Kauermann2001note} also offer the following further approximation for the critical value $z_{\tilde\alpha}$:
\[
z_{\tilde\alpha} = F_t^{-1}\left(1 - \frac{\alpha}{2}; n - p\right) + \frac{\left(z_\alpha^3 + z_\alpha\right)}{4}\left(\frac{1}{\nu} - \frac{\left(\sum_{i=1}^n g_i^2\right)^2}{n - p}\right). \]
...
\nu_R &= 2\left(\sum_{i=1}^n g_i^2 \sigma_i^2\right)^2 \left(\sum_{i=1}^n g_i^4 \sigma_i^4\right)^{-1}
\end{align*}
Rothenberg's Edgeworth approximation is then given by 
\[
\Pr\left(T^{HC} \leq t \right) \approx \Phi\left[t \left(1 - \frac{1 + t^2}{2 \nu_R} + \frac{a\left(t^2 - 1\right) + b}{2}\right)\right]. 
\]
...
\begin{equation}
\label{eq:edge_Roth}
t_\alpha = z_{\alpha}\left[1 + \frac{z_{\alpha}^2 + 1}{2 \nu} - \frac{a\left(z_{\alpha}^2 - 1\right) + b}{2}\right].
\end{equation}\todo{Is this right??? Compare to Kauermann and Carroll's approximate CI.}
...
\nu_R = \frac{2\left(\sum_{i=1}^n g_i^2 \right)^2}{\sum_{i=1}^n g_i^4}.
\]
Using the "model-based" estimates of the adjustment quantities may be a reasonable alternative, considering that if the bias of $V^{HC}$ could be well-estimated empirically, one could simply correct the estimator itself. 

\subsection{Saddlepoint approximation}

\citet{McCaffrey2006improved} developed small-sample adjustments to test statistics based on cluster-robust variance estimators, of which HC variance estimators are a special case. 
They consider both a Satterthwaite approximation (similar to Lipsitz et al.) and a saddlepoint approximation for the distribution of the test statistic, finding that the latter produced tests with more accurate size. 
The saddlepoint approximation is obtained as follows. 
Observe that the cumulative distribution of $T^{HC}$ can be expressed as \[
...
Note that $\left(\bm{c}\bs{\hat\beta} - k\right)^2 / \Var\left(\bm{c}\bs{\hat\beta}\right) \sim \chi^2_1$ and that $V^{HC}$ is distributed as a weighted sum of $\chi^2_1$ random variables, as in Equation (\ref{eq:weighted_sum_chi_sq}). 
...
It then follows that the $\Pr\left(T^{HC} \leq t\right)$ can be expressed as $\Pr(Z \leq 0)$, where $Z = \sum_{i=0}^{n-p} \gamma_i Z_i$,
$\gamma_0 = 1$, $\gamma_i = -t^2 \lambda_i / \sum_{j=1}^{n - p} \lambda_j$ for $i = 1,...,n - p$, and $Z_0,...,Z_{n-p} \iid \chi^2_1$.

The saddlepoint technique is a means to approximate the distribution of $Z$.\todo{Cite} 
Let $s$ be the saddlepoint, defined implicitly as the solution to \[
\sum_{i=0}^{n-p} \frac{\gamma_i}{1 - 2 \gamma_i s} = 0. \]
Note that the solution to the saddlepoint equation will be in the range $\left(\left(2 \min\left\{\gamma_0,...,\gamma_{n-p}\right\}\right)^{-1}, 0 \right)$ if $\sum_{i=0}^{n-p} \gamma_i > 0$ and in the range $\left(0, \left(2 \max\left\{\gamma_0,...,\gamma_{n-p}\right\}\right)^{-1}\right)$ if $\sum_{i=0}^{n-p} \gamma_i \leq 0$. 
Define the quantities $r$ and $q$ as \[
r = \text{sign}(s)\sqrt{2sz + \sum_{i=0}^{n-p} \log\left(1 - 2\gamma_i s\right)}, \qquad q = s \sqrt{2 \sum_{i=0}^{n-p} \frac{\gamma_i^2}{\left(1 - 2 \gamma_i s\right)^2}} \]
for a constant $z$. 
The saddlepoint approximation is then 
\begin{equation}
\label{eq:saddlepoint_approx}
\Pr(Z \leq z) \approx \begin{cases}\Phi(r) + \phi(r)\left[r^{-1} - q^{-1}\right] & s \neq 0 \\
\frac{1}{2} + \frac{\sum_{i=0}^n \gamma_i^3}{3 \sqrt\pi \left(\sum_{i=0}^n \gamma_i^2\right)^{3/2}} & s = 0. \end{cases}
\end{equation}\todo{Cite}
Given an observed value for the $t$-statistic $t^{HC}$, a $p$-value for $H_0$ can be calculated by taking $\gamma_i = -\left(t^{HC}\right)^2 \lambda_i / \sum_{j=1}^n \lambda_j$ for $i = 1,...,n$, finding $s$, $r$, and $q$, and evaluating $1 - \Pr(Z \leq 0)$ using Equation (\ref{eq:saddlepoint_approx}).\todo{Note numerical truncation method.}

The eigenvalues of the matrix $\bm{A}\left(\bm{I} - \bm{H}\right)\bs\Sigma\left(\bm{I} - \bm{H}\right)$ involve the unknown error variances, and thus must be estimated in order to apply the saddlepoint approximation. 
\citet{McCaffrey2006improved} suggest doing so based on a working model. 
...
For sake of completeness, the simulation studies examined the performance of both McCaffrey and Bell's working model approach and the empirical approach to calculating the saddlepoint approximation.

\subsection{Remarks}

All three/four approximations invoke the assumption that the errors are normally distributed.
Most approximations also make the assumption that $V^{HC}$ is independent of the OLS coefficient estimator, which will not hold unless the errors are homoskedastic.
The approximations may differ in the extent to which they rely on these assumptions, and thus the extent to which their performance suffers under data-generating models with non-normal or heteroskedastic errors.
Some versions of the approximations involve a working model, and it is unclear how discrepancies between the working model and the true data generating model will affect their performance. 
