...
HCCMEs were introduced in the statistics literature by \citet{Huber1967behavior} and \citet{Eicker1967limit}, in the survey sampling literature by \citet{fuller1975regression}\todo{Kish \& Frankel, 1974?}, and in econometrics by \citet{White1980heteroskedasticity}. 
...
HCCMEs are a special case of the general class of cluster-robust covariance matrix estimators (CRCMEs), also known as sandwich estimators or linearization estimators, which are commonly used in regression analysis of multi-stage survey data \citep{skinner1989domain}, econometric panel data models \citep{Arellano1987computing, white1984asymptotic}, and generalized estimating equations for longitudinal data \citep{Liang1986longitudinal}. 
...
\end{equation}
Kauermann and Carroll focus on the HC2 variance estimator and calculate its degrees of freedom based on the working assumption that the errors are actually homoskedastic, as $\nu_H$ from Equation (\ref{eq:nu_hom_HC2}). An alternative would be to use the empirical degrees of freedom estimate, $\nu_E$, from Equation(\ref{eq:nu_empirical}). 
...
The saddlepoint technique is a means to approximate the distribution of a random variable based on its cumulant generating function.
The test proposed by \citet{McCaffrey2006improved} is derived by first representing $\left|T^{HC}\right|$ as a weighted sum of independent $\chi^2_1$ variates, then approximating its cumulative distribution using the \citet{Lugannani1980saddlepoint} saddlepoint formula. 
