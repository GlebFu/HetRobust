...
\title{Heteroskedasticity-robust tests in linear regression: A review and evaluation of small-sample corrections}
\shorttitle{Small-sample HC tests}
\author{James E. Pustejovsky and Gleb Furman}
\leftheader{Pustejovsky & Furman}
\affiliation{The University of Texas at Austin}

\abstract{In linear regression models estimated by ordinary least squares, it is often desirable to use hypothesis tests and confidence intervals that remain valid in the presence of heteroskedastic errors. Wald tests based on heteroskedasticity-consistent covariance matrix estimators are a well known and widely used method that remains asymptotically valid under heteroskedasticity of an unspecified form. Wald-type t-tests based on HCCMEs maintain nominal rejection rates when the sample size is large, but they are not always accurate with small samples, and it can be difficult to determine whether a given sample is large enough to trust the asymptotic approximation. This paper reviews several approaches to approximating the sampling distribution of HCCME t-tests and thereby improving the accuracy of rejection rates in small samples. Using simulations, we investigate the relative performance of Satterthwaite, Edgeworth, and saddlepoint approximations under several data generating processes. We find that certain distributional approximations perform comparably to or better than conventional tests based on HCCMEs, with marked improvements when the nominal Type-I error rate is less than .05.}
...
\authornote{James E. Pustejovsky, Department of Educational Psychology, University of Texas at Austin; Gleb Furman, Department of Educational Psychology, University of Texas at Austin.

An earlier version of this paper was presented at the 2017 annual convention of the American Educational Research Association in Chicago, IL.

Correspondence concerning this article should be addressed to James E. Pustejovsky, University of Texas at Austin, Department of Educational Psychology, 1912 Speedway, Stop D5800, Austin, TX 78712-1289. Email: pusto@austin.utexas.edu.
}
...
Also, existing simulation evidence about these tests has focused almost exclusively on Type-I error rates at the nominal $\alpha = .05$ level. 
Furthermore, it appears that the existing approximations have been developed in isolation, without reference to previous work, and they have received little subsequent attention \citep[e.g., none are discussed in a recent review of heteroskedasticity-robust inference by][]{MacKinnon2013thirty}. 
...
Though not comprehensive, our simulations include several different distributional models, varying degrees of heteroskedasticity, varying degrees of leverage (which is known to have a strong influence on the performance of HCCMEs), and several nominal Type-I error rates, including levels lower than $\alpha = .05$.
Lower $\alpha$ levels are of interest because scholars have recently argued for reducing the conventional $\alpha = .05$ level to level of $.005$ \citep{benjamin2018redefine}. 
Even under current conventions, lower $\alpha$ levels are important for analyses involving multiple comparisons corrections 
...
y_i = \mat{x}_i\bs\beta + \epsilon_i,
\end{equation}
for $i = 1,...,n$, where $y_i$ is the outcome, $\mat{x}_i$ is a $1 \times p$ row-vector of covariates (including an intercept) for observation $i$, $\bs\beta$ is a $p \times 1$ vector of regression coefficients, and $\epsilon_i$ is a mean-zero error term with variance $\sigma_i^2$. 
We shall assume that the errors are mutually independent. 
Let $\mat{y} = \left(y_1,...,y_n\right)'$ denote the $n \times 1$ vector of outcomes, $\mat{X} = \left(\mat{x}_1',...,\mat{x}_n'\right)'$ be the $n \times p$ design matrix, and $\bs\epsilon$ be the $n \times 1$ vector of errors with $\E\left(\bs\epsilon\right) = \mat{0}$ and $\Var\left(\bs\epsilon\right) = \bs\Sigma = \diag\left(\sigma_1^2,...,\sigma_n^2\right)$. 
Let $\mat{M} = \left(\mat{X}'\mat{X} / n\right)^{-1}$. 
The OLS estimator of $\bs\beta$ is then given by 
\begin{equation}
\label{eq:beta-hat}
\bs{\hat\beta} = \mat{M}\mat{X}'\mat{y} / n.
\end{equation}
Let $\mat{I}$ denote an $n \times n$ identity matrix and $\mat{H} = \mat{X}\mat{M}\mat{X}' / n$ denote the hat matrix, which has entries $h_{ij} = \mat{x}_i \mat{M}\mat{x}_j' / n$. 
Finally, let $e_i = y_i - \mat{x}_i \bs{\hat\beta}$, $i = 1,...,n$ denote the residuals. 

In what follows, the aim will be to test a hypothesis regarding a linear combination of the regression coefficients, expressed as $H_0: \mat{c}'\bs\beta = k$, with target Type-I error rate $\alpha$. 
All tests under consideration are based on the Wald statistic
\begin{equation}
T = \frac{\mat{c}'\bs{\hat\beta} - k}{\sqrt{V}},
\end{equation}
where $V$ is some estimator for $\Var\left(\mat{c}'\bs{\hat\beta}\right)$. In what follows, we shall use superscripts on $T$ that correspond to the superscript for the variance estimator used to calculate it.
...
The variance of $\mat{c}'\bs\beta$ is then estimated by $V^{hom} = \hat\sigma^2 \mat{c}'\mat{M}\mat{c} / n$, where $\hat\sigma^2 = \sum_{i=1}^n e_i^2 / (n - p)$. 
...
\begin{equation}
\label{eq:var_beta}
\Var\left(\mat{c}'\bs{\hat\beta}\right) = \frac{1}{n} \mat{c}'\mat{M} \left(\frac{1}{n}\sum_{i=1}^n \sigma_i^2 \mat{x}_i\mat{x}_i'\right) \mat{M}\mat{c}
\end{equation}
The HCCCMEs estimate $\Var\left(\mat{c}'\bs{\hat\beta}\right)$ by replacing the $\sigma_i^2$ with estimates based on the squared residuals. All of the HCCMEs have the same general form  
\begin{equation}
\label{eq:sandwich}
V^{HC} = \frac{1}{n} \mat{c}'\mat{M} \left(\frac{1}{n}\sum_{i=1}^n \omega_i e_i^2 \mat{x}_i\mat{x}'\right)\mat{M} \mat{c}
\end{equation}
...
In HC5, the truncation point depends on the maximum leverage value and will exceed 4 if the maximum leverage value is more than \Sexpr{round(4 / .7, 2)} times the average. 

For any of the HCCMEs and under weak conditions on the error and covariate distributions, the robust Wald statistic $T^{HC} = \left(\mat{c}'\bs{\hat\beta} - k\right) / \sqrt{V^{HC}}$ converges in distribution to $N(0,1)$ as $n$ increases to infinity. 
...
\[
V^{HC} = \sum_{i=1}^n \omega_i \left(g_i e_i\right)^2 = \mat{e}' \mat{A} \mat{e} = \bs\epsilon' \mat{B} \bs\epsilon , \]
where $g_i = \mat{x}_i \mat{M} \mat{c} / n$, $\mat{A} = \diag\left(\omega_1 g_1^2,...,\omega_n g_n^2\right)$, and $\mat{B} = \left(\mat{I} - \mat{H}\right)\mat{A}\left(\mat{I} - \mat{H}\right)$ \citep{Bell2002bias, Cribari-Neto2011new}. 
It follows from the properties of quadratic forms that 
\begin{equation}
\label{eq:V_expectation}
\E\left(V^{HC}\right) = \tr\left[\mat{B}\bs\Sigma\right]. 
\end{equation}
Furthermore, assuming that the model errors are normally distributed, the variance of the quadratic form is
\begin{equation}
\label{eq:V_variance}
\Var\left(V^{HC}\right) = 2\tr\left[\mat{B} \bs\Sigma \mat{B} \bs\Sigma\right] = 2\tr\left[\mat{B} \left(\mat{B} \circ \mat{S}\right)\right], 
\end{equation}
where $\circ$ denotes the element-wise (Hadamard) product and $\mat{S}$ has entries $S_{ij} = \sigma_i^2 \sigma_j^2$ \citep{Lipsitz1999degrees}. 
...
Note that the matrix $\mat{B}\bs\Sigma$ has rank $n - p$. 
...
Specifically, let $\mat{\hat{S}}$ be the matrix with entries \[
...
to be used as an estimate of $\mat{S}$ in Equation (\ref{eq:V_variance}).
The empirically estimated degrees of freedom are then given by
\begin{equation}
\label{eq:nu_empirical}
\nu_E = \frac{\left(V^{HC}\right)^2}{\tr\left[\mat{B} \left(\mat{B} \circ \mat{\hat{S}}\right)\right]}.
\end{equation}
...
In the present context, a leading candidate for a working model is to assume that the errors are homoskedastic, so that $\bs\Sigma = \sigma^2 \mat{I}$.
...
\citet{Kauermann2001note} proposed approximate confidence intervals for $\mat{c}'\bs{\hat\beta}$ based on an Edgeworth approximation to the distribution of $T^{HC}$. 
Their approximation is based on the assumption that $V^{HC}$ is unbiased and independent of $\mat{c}'\bs{\hat\beta}$. 
...
\end{equation}
This further approximation is convenient for calculating a confidence interval for $\mat{c}'\bs{\hat\beta}$ because it avoids the need to numerically solve Equation (\ref{eq:Kauermann_crit}). 
...
Prior to \citet{Kauermann2001note}, \citet{Rothenberg1988approximate} developed an Edgeworth approximation for the distribution of $T^{HC}$, calculated using the HC0 variance estimator. Rothenberg's approximation differs from Kauermann and Carroll's in two key ways. First, it allows for the possibility that $V^{HC}$ is a biased estimator of $\Var\left(\mat{c}'\bs{\hat\beta}\right)$; such will be the case for $V^{HC0}$ if the errors are homoskedastic, for instance. Second, it allows for the possibility of dependence between $\mat{c}'\bs{\hat\beta}$ and $V^{HC}$, which arises when the errors are \textit{not} homoskedastic.
...
\]
Here, the $a$ term measures covariance between $\mat{c}'\bs{\hat\beta}$ and $V^{HC}$; the $b$ term measures the relative bias of $V^{HC}$; and $\nu$ is the Satterthwaite degrees of freedom. 
...
\Pr\left(\left|T^{HC}\right| \leq t\right) = \Pr\left(\frac{\left(\mat{c}\bs{\hat\beta} - k\right)^2}{\Var\left(\mat{c}\bs{\hat\beta}\right)} - t^2 \frac{V^{HC}}{\Var\left(\mat{c}\bs{\hat\beta}\right)} \leq 0\right). \]
Observe that $\left(\mat{c}\bs{\hat\beta} - k\right)^2 / \Var\left(\mat{c}\bs{\hat\beta}\right) \sim \chi^2_1$ and that $V^{HC}$ is distributed as a weighted sum of $\chi^2_1$ random variables, as in Equation (\ref{eq:weighted_sum_chi_sq}). 
\citet{McCaffrey2006improved} assume that $V^{HC}$ is unbiased, so that \[
\E\left(V^{HC}\right) = \tr\left(\mat{B}\bs\Sigma\right) = \sum_{j=1}^{n - p} \lambda_j, \]
...
In practice, the unknown error variances must be estimated in order to find the eigenvalues of $\mat{B}\bs\Sigma$. 
\citet{McCaffrey2006improved} propose to do so based on a working model. 
For instance, assuming that the errors are homoskedastic implies that the eigenvalues of $\mat{B}$ may be used in the saddlepoint calculations. 
An alternative, not considered by \citet{McCaffrey2006improved}, would be to use the eigenvalues of $\mat{B}\bs{\hat\Sigma}$, where $\bs{\hat\Sigma} = \diag\left(e_1^2,...,e_n^2\right)$.
...
This section reports a large simulation study that investigate the performance the distributional approximations under a range of conditions, including conditions in which errors are non-normally distributed. 
% The simulations are based on a model described by \citet{Long2000using}, which was designed to emulate the features of real empirical data in the social sciences.
...
size_results_select %>%
  filter((test=="naive" & HC %in% c("HC3","HC4")) | test %in% selected_tests, n == 25) %>%
  filter(reject_rel <= rel_reject_ceiling) %>%
  size_plot()
...
size_results_select %>%
  filter((test=="naive" & HC %in% c("HC3","HC4")) | test %in% selected_tests, n == 50) %>%
  filter(reject_rel <= rel_reject_ceiling) %>%
  size_plot()
...
size_results_select %>%
  filter((test=="naive" & HC %in% c("HC3","HC4")) | test %in% selected_tests, n == 100) %>%
  filter(reject_rel <= rel_reject_ceiling) %>%
  size_plot()
...
Our review of HCCME-based hypothesis-testing procedures was also limited in scope. 
Whereas we considered the case where the design matrix has fixed dimension, recent work by \citet{cattaneo2018inference} proposed HCCMEs that are asymptotically valid when the number of predictors grows with sample size. 
Another important class of techniques for approximating the distribution of test statistics based on HCCMEs, not considered here, is via bootstrap resampling. 
