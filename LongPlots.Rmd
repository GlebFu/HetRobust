...
df %>%
  filter(criterion == "size") %>%
  group_by(HC, test, alpha) %>%
  summarize(atBelNom = round(mean(MCSE(alpha) + alpha >= p & alpha - MCSE(alpha) <= p) *100 ,2)) %>%
  spread(key = test, value = atBelNom) %>%
  filter(alpha == .100)
...
```{r, echo = F, warning = F, message = F}
pv <- .050
df2 <- merge(df, filter(df, HC == "HC3") %>% mutate(HC3 = p) %>% select(-HC), all = T)

df2 %>%
  filter(criterion == "size") %>%
  group_by(test, HC, n) %>%
  summarize(beatHC3 = mean(abs(p - alpha) <= abs(HC3 - alpha), na.rm = T))
  spread(key = n, value = beatHC3)
...
lapply(levels(df$Edist), nbygroup, data = df, hc = "HC2")
