...
nbygroup <- function(data, hc = "HC3", dist = "En", pv = .005, xs = c("x0", "x1", "x2", "x3", "x4", "x5")) {
  ttl <- paste("Figure 1. ", hc, " rejection rates when ", expression(alpha), " = ", pv, sep = "")
  
  data %>%  
    filter(HC == hc, criterion == "size", Edist == dist, alpha == pv, coef %in% xs) %>%
    ggplot(aes(y = p,
