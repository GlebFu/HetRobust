...
\[
Y_i = \beta X_i + \exp(\zeta X_i) \epsilon_i, \quad i = 1,...,n.
\]
where the errors $\epsilon_1,...,\epsilon_n$ were simulated from one of three different error distributions, including the standard normal, $t_5$ (scaled to have unit variance), or $\chi^2_5$ (centered and scaled) distribution. We used the skedasticity function 
\[
\sigma(\zeta, X_i) = (1 - 4 \zeta)^\zeta \exp(2 \zeta X_i),
\]
so that $\E\left[\sigma^2(\zeta, X_i)\right] = 1$.

\citet{MacKinnon2013thirty} considered a regression with four log-normally distributed predictors, in which some observations have very high leverage. 
