...
filter(results, HC == "HC3", criterion == "size", test != "naive") %>%
  select(n:Edist, coef, test, p05) %>%
  merge(HC3) %>%
  mutate(comp = as.factor(apply(cbind(x = naive, y = p05), 1, compare))) %>%
  ggplot(aes(x = naive,
             y = p05,
             color = comp)) +
    geom_point(size = 1) +
...
filter(results, HC == "HC1", criterion == "size") %>%
  select(n:Edist, coef, test, p05) %>%
  merge(HC3) %>%
...
filter(results, HC == "HC2", criterion == "size") %>%
  select(n:Edist, coef, test, p05) %>%
  merge(HC3) %>%
  mutate(comp = unlist(apply(cbind(x = naive, y = p05), 1, compare))) %>%
  ggplot(aes(x = naive,
...
filter(results, HC == "HC3", criterion == "size") %>%
  select(n:Edist, coef, test, p05) %>%
  merge(HC3) %>%
  mutate(comp = unlist(apply(cbind(x = naive, y = p05), 1, compare))) %>%
  ggplot(aes(x = naive,
